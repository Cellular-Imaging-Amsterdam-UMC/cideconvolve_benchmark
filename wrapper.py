...
BENCH_METHOD_SETS = {
    "sdeconv_rl, pycudadecon_rl_cuda": _BENCH_BASE,
    "sdeconv_rl, pycudadecon_rl_cuda, redlionfish_rl": _BENCH_BASE_RLF,
    "sdeconv_rl, pycudadecon_rl_cuda, deconwolf_rl, deconwolf_shb, redlionfish_rl": _BENCH_BASE_DW_RLF,
    "sdeconv_rl, pycudadecon_rl_cuda, deconwolf_rl, deconwolf_shb, deconvlab2_rl, deconvlab2_rltv, redlionfish_rl": _BENCH_BASE_DW_DL2_RLF,
    "all": _BENCH_ALL,
}
...
def _make_benchmark_montage(
    ...
    for nit in bench_iterations:
        ...
        for method, variant in col_order:
            ...
            if met is not None:
                gpu_d = met['torch_gpu_delta_mb'] if metrics_key.startswith('sdeconv_') else met['gpu_mem_delta_peak_mb']
                label = (f"{label_name}\n"
                         f"{nit} iter  {met['time_s']:.1f}s\n"
                         f"Alloc \u0394{_format_bytes(gpu_d)}"
                         f"  RAM \u0394{_format_bytes(met['ram_delta_peak_mb'])}")
            else:
                label = f"{label_name}\n{nit} iter"
