"""Vectorial spherical propagator."""
import math
import typing as tp
...
class VectorialSphericalPropagator(SphericalPropagator):
    r"""
    Propagator for the vectorial case of the Richard's-Wolf integral
    in spherical parameterization.

    Parameters
    ----------
    e0x : float
        Initial electric field component x. Default ``1.0``.
    e0y : float
        Initial electric field component y. Default ``0.0``.
    """
    ...
    def compute_focus_field(self) -> torch.Tensor:
        ...
        batched = vmap(self._compute_psf_at_defocus,
                       in_dims=(0, None, None, None, None, None, None))
        return batched(self.defocus_filters, J0, J1, J2, self.get_pupil(), sin_t, cos_t)

    def _compute_psf_at_defocus(self, defocus_term, J0, J1, J2, pupil, sin_t, cos_t):
